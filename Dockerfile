...
COPY --chown=cadence:cadence --chmod 0700 docker/entrypoint.sh /docker-entrypoint.sh
...
COPY --chown=cadence:cadence docker/config_template.yaml /etc/cadence/config/docker.yaml

WORKDIR /etc/cadence

ENV CADENCE_ROOT=/etc/cadence
ENV CADENCE_CONFIG_DIR=config
ENV CADENCE_ENVIRONMENT=docker
ENV CADENCE_SERVICES="history,matching,frontend,worker"
...
CMD ["/usr/local/bin/cadence-server", "start"]


# All-in-one Cadence server (~450mb)
FROM cadence-server AS cadence-auto-setup

ENV CADENCE_AUTO_SETUP=true
# In case we're using Cassandra
ENV CADENCE_SETUP_OPTION_REPLICATION_FACTOR=1
